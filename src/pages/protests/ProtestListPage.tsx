...
export function ProtestListPage() {
  ...
  const filtered = statusFilter ? protests.filter(p => p.status === statusFilter) : protests

  const getDriver = (id: string) => drivers.find(d => d.id === id)?.nickname || id
  const getCompetition = (id: string) => {
    const c = competitions.find(x => x.id === id)
    return c ? getName(c, lang) : id
  }
  ...
  const columns = [
    ...
    { key: 'competition', header: t('protest.competition'), render: (p: typeof protests[0]) => <span className="text-sm">{getCompetition(p.competitionId)}</span> },
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 text-sm">
          <span className="px-2 py-1 bg-yellow-100 text-yellow-700 rounded">{pendingCount} {t('protest.status.pending')}</span>
          <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded">{reviewingCount} {t('protest.status.reviewing')}</span>
        </div>
      </div>
      <Card>
